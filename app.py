...
def index():
    ...
    user_data = db.execute("SELECT symbol, SUM(shares) AS totala FROM activity WHERE a_id = ? GROUP BY symbol", session["user_id"])
    ...
    for d in user_data:
        dat = lookup(d["symbol"])
        com_name = dat["name"]
        d["shares"] = round(d["totala"])
        stock_price = dat["price"]
        d["name"] = com_name
        d["price"] = stock_price
        d["totals"] = stock_price * d["totala"]
        total = total + d["totals"]
...
def buy():
    ...
    if request.method == "POST":
        ...
        new_cash = round(user_current_cash - multiplier, 2)
